...
export default function PitchDeck({ params }: { params: { company: string } }) {
  const [mounted, setMounted] = useState(false);
  const companyName = params.company.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: '1000px', margin: '0 auto', padding: '4rem 2rem' }}>
        ...
        >
          ...
          <p style={{ fontSize: '1.25rem', color: '#a1a1aa', maxWidth: '600px', margin: '0 auto', lineHeight: 1.6 }}>
            Bypass the 3-month hiring cycle. Deploy an elite fractional team tomorrow and hit your scaling targets immediately.
          </p>
        </div>

        {/* The ROI Calculator / Salary Arbitrage Widget */}
        <div 
        ...
        >
          <h3 style={{ fontSize: '1.5rem', fontWeight: 600, marginBottom: '2rem', textAlign: 'center' }}>The Financial Arbitrage</h3>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '2rem' }}>
            ...
            <div style={{ background: 'rgba(255, 95, 86, 0.05)', border: '1px solid rgba(255, 95, 86, 0.2)', borderRadius: '16px', padding: '2rem', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ fontSize: '2.5rem', fontWeight: 700, color: '#fff', marginBottom: '1rem' }}>$125k<span style={{ fontSize: '1rem', color: '#888', fontWeight: 400 }}>/yr</span></div>
              <ul style={{ listStyle: 'none', padding: 0, margin: 0, color: '#a1a1aa', fontSize: '0.9rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <li style={{ display: 'flex', gap: '8px' }}><TrendingDown size={16} color="#ff5f56" /> + 20% Benefits & Taxes</li>
                <li style={{ display: 'flex', gap: '8px' }}><Clock size={16} color="#ff5f56" /> 45-90 Day Hiring Cycle</li>
            ...
            <div style={{ background: 'linear-gradient(180deg, rgba(39, 201, 63, 0.1) 0%, rgba(39, 201, 63, 0.02) 100%)', border: '1px solid rgba(39, 201, 63, 0.3)', borderRadius: '16px', padding: '2rem', position: 'relative', overflow: 'hidden' }}>
              ...
              <div style={{ fontSize: '2.5rem', fontWeight: 700, color: '#fff', marginBottom: '1rem' }}>$48k<span style={{ fontSize: '1rem', color: '#888', fontWeight: 400 }}>/yr</span></div>
              <ul style={{ listStyle: 'none', padding: 0, margin: 0, color: '#a1a1aa', fontSize: '0.9rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <li style={{ display: 'flex', gap: '8px' }}><CheckCircle2 size={16} color="#27c93f" /> Zero Benefits/Taxes</li>
          ...
          <div style={{ textAlign: 'center', marginTop: '2.5rem', padding: '1.5rem', background: 'rgba(255,255,255,0.03)', borderRadius: '12px' }}>
            <h4 style={{ fontSize: '1.25rem', color: '#fff', margin: '0 0 0.5rem 0' }}>Net Savings: <span style={{ color: '#27c93f', fontSize: '1.5rem' }}>$77,000+ per year</span></h4>
            <p style={{ fontSize: '0.9rem', color: '#888', margin: 0 }}>While deploying 3x faster than traditional hiring.</p>
          </div>
        </div>
        ...
        >
          ...
          <a href="#" style={{ background: '#fff', color: '#000', textDecoration: 'none', padding: '1rem 2.5rem', borderRadius: '999px', fontSize: '1.1rem', fontWeight: 700, display: 'inline-flex', alignItems: 'center', gap: '8px', transition: 'transform 0.2s' }} onMouseOver={e => e.currentTarget.style.transform = 'scale(1.05)'} onMouseOut={e => e.currentTarget.style.transform = 'scale(1)'}>
            <Calendar size={20} />
            Book Discovery Call
          </a >
        </div>
